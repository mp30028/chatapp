package com.zonesoft.chatapp.routing_tests;
...
import com.zonesoft.chatapp.configurations.RouterConfiguration;
...
class PersonsEndPointTests {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(PersonsEndPointTests.class);
	...
	void test_get_persons_returnsOK_withPersonsList() {
		...
		client
			.get()
			.uri(uriBuilder -> uriBuilder.path("/router/persons").build())
			.exchange()
			.expectStatus()
			.isOk()
			.expectBody()
	...
	void test_get_personById_returnsOK_withPerson() {
		...
		client
			.get()
			.uri(uriBuilder -> uriBuilder.path("/router/persons/{id}").build(selectedPerson.getId()))
